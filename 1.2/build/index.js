 ...
 * @module found
 **/
KISSY.add('gallery/sapper/1.2/index',function (S, Node, Base, UA) {
    var EMPTY = '';
    var $ = Node.all;
    var Sapper = Base.extend({
        ...
        _bindUI: function () {
            ...
            container.delegate('keyup', '.J_Sapper_Input',function(e){
                self._handleKeyup(e);
            });
            container.delegate('keydown', '.J_Sapper_Input', function(e){
        ...
        _searchText: function (text) {
            ...
            var localdata =  self.get('localdata');
            result = self._filterResult(localdata,text);
         ...
         * @private
         */
        _filterResult:function(nodeList,text){
            var self = this;
            var success = false;
            var result = [];
            nodeList.each(function(node){
                ...
                var html = node.html();
                var hasSearchText = html.search(text) !== -1;
                if(isLink && hasSearchText) {
                    result.push(node);
                }
            });
            return result;
        ...
        "scope": {  /*查找范围，按selector方式给出,默认为body a*/
            "value": 'body a'
        }

    }});
    return Sapper;
}, {requires: ['node', 'base',  'ua', './index.css']});
